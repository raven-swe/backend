...
describe('SettingsController', () => {
  ...
  describe('GET /users/me/settings/mutes', () => {
    const mockUser = { id: '1' };

    it('should return mutes with default limit (20) when no limit provided', async () => {
      // Arrange
      ...
      mockSettingsService.getUserMutedUsers.mockResolvedValue(mockServiceResult);

      // Act
      const result = await controller.getUserMutedUsers(mockUser, undefined, undefined);

      // Assert
      expect(mockSettingsService.getUserMutedUsers).toHaveBeenCalledWith(
        BigInt(1),
        20, // default limit
        undefined, // no cursor
      );
    ...
    it('should return muted users with custom limit when provided', async () => {
      // Arrange
      ...
      mockSettingsService.getUserMutedUsers.mockResolvedValue(mockServiceResult);
      // Act
      const result = await controller.getUserMutedUsers(mockUser, customLimit, undefined);
    ...
    it('should return muted users with cursor for pagination', async () => {
      // Arrange
      ...
      // Act
      const result = await controller.getUserMutedUsers(mockUser, undefined, cursor);
    ...
    it('should use default limit (20) when invalid limit provided', async () => {
      ...
      for (const invalidLimit of invalidLimits) {
        await controller.getUserMutedUsers(mockUser, invalidLimit, undefined);
    ...
    it('should transform items to CompactUserDto instances', async () => {
      ...
      // Assert
      expect(result.items).toHaveLength(2);
      expect(result.items[0]).toHaveProperty('username', 'muted1');
      expect(result.items[0]).toHaveProperty('displayName', 'Muted One');
    });

    it('should return empty items array when user has no muted users', async () => {
      // Arrange
      const mockServiceResult = {
        items: [],
        pagination: {
      ...
      // Assert
      expect(mockSettingsService.getUserMutedUsers).toHaveBeenCalledWith(BigInt(1), 20, undefined);
    ...
    it('should handle large limit values correctly', async () => {
      ...
      // Act
      const result = await controller.getUserMutedUsers(mockUser, largeLimit, undefined);

      // Assert
      expect(mockSettingsService.getUserMutedUsers).toHaveBeenCalledWith(BigInt(1), 100, undefined);
    ...
    it('should pass through service errors (user not found)', async () => {
      ...
      mockSettingsService.getUserMutedUsers.mockRejectedValue(error);

      // Act & Assert
      await expect(controller.getUserMutedUsers(mockUser, undefined, undefined)).rejects.toThrow(
        'User not found',
      );
    });
    ...
    it('should pass through service errors (invalid cursor)', async () => {
      ...
      // Act & Assert
      await expect(
        controller.getUserMutedUsers(mockUser, undefined, invalidCursor),
    ...
    it('should correctly convert user id string to BigInt', async () => {
      ...
      // Act
      await controller.getUserMutedUsers({ id: largeUserId }, undefined, undefined);
